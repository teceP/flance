...
import { Calendar, Check, DollarSign, FileText, MoreHorizontal, Plus, Trash2, X } from "lucide-react"
import { createBrowserClient } from "@/lib/pocketbase";
import { ClientsRecord, InvoicesRecord } from '@/types/pocketbase-types'
...
const invoiceFormSchema = z.object({
  ...
  status: z.string().nonempty("Status is not set."),
  footer: z.string(),
});
...
const defaultValues: Partial<InvoiceFormValues> = {
  ...
  status: "not_sent",
  footer: "Thank you for your trust. Feel free to contact us if you have any questions.",
};
...
export default function CreateInvoicePage() {
  ...
  const [status, setStatus] = useState<string[]>(["not_sent", "sent", "paid", "canceled"]);
  const [invoiceNumber, setInvoiceNumber] = useState('0001'); // Initiale Nummer
  ...
  const onSubmit = async (data: InvoiceFormValues) => {
    ...
    const invoiceNumberWithPrefix = invoiceNumberPrefix + data.invoiceNumber;

    // Schritt 1: Erstelle die Items
    const createdItemIds = await createInvoiceItems(data.items);

    // Schritt 2: Erstelle die Rabatte (optional)
    const discountIds = await createInvoiceDiscounts(data.discounts);

    // Schritt 3: Erstelle die Rechnung mit den IDs
    const modelData = {
      invoice_number: invoiceNumberWithPrefix,  // Rechnungsnummer
      issue_date: data.issueDate.toISOString(),  // Ausstellungsdatum im ISO-Format
      due_date: data.dueDate.toISOString(),  // Fälligkeitsdatum im ISO-Format
      subtotal: data.subtotal,  // Zwischensumme
      tax: data.tax.reduce((acc, taxItem) => acc + taxItem.amount, 0),  // Steuerbetrag summieren
      total: data.total,  // Endbetrag
      text: "Invoice Details",  // Optionaler Text
      client_id: selectedClient,  // Kunden-ID aus dem Formular
      invoice_item_ids: createdItemIds,  // IDs der erstellten Rechnungsartikel
      invoice_discount_ids: discountIds,  // IDs der Rabatte
      user_id: pb.authStore.model?.id,  // Benutzer-ID des aktuell angemeldeten Benutzers
      status: data.status,  // Status der Rechnung
      is_draft: data.status === "not_sent",  // Entwurfstatus (wenn nicht gesendet)
      footer: data.footer,  // Fußzeile
    };

    // Anlegen der Rechnung in der PocketBase-Datenbank
    try {
      const createdInvoice = await pb.collection("invoices").create(modelData);
  ...
  // Funktion zum Erstellen von Rabatten (optional)
  const createInvoiceDiscounts = async (discounts: any[]) => {
    const createdDiscountIds: string[] = [];
    try {
      for (const discount of discounts) {
        const createdDiscount = await pb.collection("invoice_discounts").create({
          description: discount.description,
          amount: discount.amount,
          rate: discount.rate,
        });
        createdDiscountIds.push(createdDiscount.id);  // Die ID jedes Rabatts in das Array pushen
      }
    } catch (error) {
      console.error("Error creating invoice discounts:", error);
    }
    return createdDiscountIds;  // IDs der erstellten Rabatte zurückgeben
  };

  // Funktion zum Erstellen von Items in PocketBase
  const createInvoiceItems = async (items: any[]) => {
    const createdItemIds: string[] = [];
    try {
      for (const item of items) {
        const createdItem = await pb.collection("invoice_items").create({
          name: item.name,
        ...
        createdItemIds.push(createdItem.id);  // Die ID jedes Items in das Array pushen
      }
    } catch (error) {
      console.error("Error creating invoice items:", error);
    }
    return createdItemIds;  // IDs der erstellten Items zurückgeben
  };
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <div className='flex items-center'>
                              <Input
                                className="rounded-l-md rounded-r-none w-[auto]"
                                placeholder='INV-'
                                disabled={true}
                                readOnly
                              />
                              <Input
                                {...field}
                                className="flex-grow rounded-r-md rounded-l-none px-3 py-1"
                                type="text" // Text wird verwendet, um die führenden Nullen zu erlauben
                                pattern="[0-9]*" // Nur Zahlen eingeben
                                inputMode="numeric" // Option für mobile Geräte, um numerische Tastatur zu zeigen
                                onInput={(e) => {
                                  const value = e.target.value;
                                  // Erlaubt nur numerische Zeichen und behält führende Nullen bei
                                  e.target.value = value.replace(/\D/g, ""); // Entfernt alle Nicht-Zahlen-Zeichen
                                }}
                                placeholder="0001"
                              />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <Select defaultValue='not_sent' onValueChange={(value) => {
                            if (value === "new") {
              ...
              <Card>
                <CardContent className="p-6">
                  ...
                  <div className="flex justify-between">
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormControl>
                              ...
                              <Textarea
                                placeholder="This text will be displayed at the pdf footer."
                                className="w-full"
                                {...field}
                              />
                            </FormControl>
                    ...
                    {/* Right side: Subtotal, Discounts, Tax, Total */}
                      ...
                      <Page size="A4" style={styles.page}>
                        ...
                        <View style={styles.header}>
                          ...
                          <View style={styles.invoiceInfo}>
                            ...
                            <Text style={styles.invoiceDetails}>Rechnungsnummer: {invoiceNumberPrefix + form.watch("invoiceNumber")}</Text>
